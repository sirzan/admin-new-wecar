...
-- Políticas de lectura pública para planes
create policy "Plans are viewable by everyone" on public.plans for select using (true);

-- Políticas para que los usuarios vean sus propias suscripciones
create policy "Users can view own subscriptions" on public.subscriptions for select using (auth.uid() = user_id);

-- Políticas para que los usuarios vean los pagos de sus suscripciones
...
-- Insertar los planes iniciales (sin el icono, como se solicitó)
insert into public.plans (id, name, price, duration, duration_weeks, max_cars, highlight, features) values
